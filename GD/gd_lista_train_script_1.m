function gd_lista_train_script_1(depth,alpha,t0,CONTINUE_TRAINING)
DATASET='USPS';
...
if (CONTINUE_TRAINING)
  load(sprintf('GD/init_mat/%s_lista_1.mat',DATASET),'network');
  LEARNING_RATE.t0=network.iter+1;
  %network.error=1;
  network=lista_train_adagrad(train_data,sp_code,network,NUM_CLASSES,...
  ...
  network.iter=LEARNING_RATE.t0;
  network=lista_train_adagrad(train_data,sp_code,network,NUM_CLASSES,...
    LEARNING_RATE,MAX_ITER,CONV_THRES,CONV_COUNT,ERROR_CHECK_ITER);
end
disp('Saving.');
save(sprintf('GD/init_mat/%s_lista_1.mat',DATASET),'network');
end